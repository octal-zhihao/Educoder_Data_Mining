...
def spamTest():
    ...
    for i in range(10):
        ...
        del(trainingSet[randIndex])                           # 在训练集中删除该索引值

    trainMat = []                                             # 创建训练集矩阵
    trainClasses = []                                         # 创建训练集类别标签
    for docIndex in trainingSet:                              # 遍历训练集
        trainMat.append(setOfWords2Vec(vocabList, docList[docIndex]))  # 将生成的词集模型添加到训练矩阵
        trainClasses.append(classList[docIndex])              # 将类别标签添加到训练类别列表
