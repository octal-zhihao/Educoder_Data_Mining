...
class DecisionTree(object):
    ...
    def predict(self, feature):
        ...
        # 单个样本分类
        def classify(tree, single_feature):
            if not isinstance(tree, dict):
                return tree
            t_index, t_value = list(tree.items())[0]
            f_value = single_feature[t_index]
            
            # 检查 t_value 是否为字典并且 f_value 是否在 t_value 的键中
            if isinstance(t_value, dict) and f_value in t_value:
                return classify(t_value[f_value], single_feature)
            else:
                # 如果f_value不在当前的分支中，返回最多的类别标签
                return max(t_value, key=t_value.get) if isinstance(t_value, dict) else t_value


        # 对所有样本进行分类预测
        results = [classify(self.tree, f) for f in feature]
